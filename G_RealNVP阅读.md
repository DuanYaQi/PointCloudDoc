...
**以隐空间Z的隐变量z采样为模型核心，以容易计算的log-likelihood损失函数为指导**
...
$$
\begin{equation}
 \begin{aligned} p_{X}(x) &=p_{Z}(f(x))\left|\operatorname{det}\left(\frac{\partial f(x)}{\partial x^{T}}\right)\right|\end{aligned} \tag{2} 
\end{equation}
$$

式 2 中描述的算法显示了如何学习无界空间上的分布。 通常，感兴趣的数据具有有界的量级。 例如，在应用推荐的抖动处理之后，图像的像素值通常位于 [0, 256] D 中。 为了减少边界效应的影响，我们改为对 logit(α+(1−α) 256 ) 的密度进行建模，其中 $α$ 在这里被选为 0.05。 我们在计算对数似然和每维位数时会考虑这种转换。 我们还在训练期间扩充 CIFAR-10、CelebA 和 LSUN 数据集，以还包括训练示例的水平翻转。

我们在四个自然图像数据集上训练我们的模型：CIFAR-10、Imagenet、大规模场景理解 (LSUN)、CelebFaces (CelebA)  属性。 更具体地说，我们在下采样到 32 × 32 和 64 × 64 版本的 Imagenet 上进行训练。 对于 LSUN 数据集，我们在卧室、塔楼和教堂户外类别上进行训练。LSUN 的过程与 [47] 中的相同：我们对图像进行下采样，使最小边为 96 像素并随机裁剪 64 × 64。对于 CelebA，我们使用与 [38] 中相同的过程：我们采用一个大约 148 × 148 的中央裁剪，然后将其调整为 64 × 64。

我们使用第 3.6 节中描述的多尺度架构，并在耦合层中使用深度卷积残差网络，具有整流器非线性和跳跃连接，如 [46] 所建议的。为了计算缩放函数 $s$，我们使用双曲正切函数乘以学习的尺度，而平移函数 t 具有仿射输出。 我们的多尺度架构递归重复，直到最后一次递归的输入是 4 × 4 × c 张量。 对于大小为 32 × 32 的图像数据集，我们使用 4 个残差块和 32 个隐藏特征图作为带有棋盘掩码的第一个耦合层。 对于大小为 64 × 64 的图像，仅使用 2 个残差块。我们使用 64 的批量大小。对于 CIFAR-10，我们使用 8 个残差块、64 个特征图，并且仅缩小一次。 我们使用具有默认超参数的 ADAM 进行优化，并在系数为 5 · 10 -5 的权重参数上使用 L2 正则化。

我们将先验 $p_Z$ 设置为各向同性单位范数高斯。 但是，任何分布都可以用于 $p_Z$，包括也在训练期间学习的分布，例如从自回归模型或（对训练目标稍作修改）变分自编码器。
