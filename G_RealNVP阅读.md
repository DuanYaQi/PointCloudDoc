...
\end{equation}\tag{9}
$$
我们使用了两个利用图像的局部相关结构的分区：空间棋盘模板和逐通道遮蔽（见**Fig. 3**）。如果空间坐标为奇数则空间棋盘模板的值为 1，否则为 0。通道方向的遮罩 $b$ 对于通道尺寸的前一半为 1，对于后一半为 0。对于此处介绍的模型，$s(·)$ 和 $t(·)$ 均为整流卷积网络。
...
我们使用**压缩 squeezing 操作**实现多尺度体系结构：对于每个通道，它将图像划分为形状为 $ 2\times 2 \times c $ 的子正方形，然后将其重塑为形状为 $ 1 \times 1 \times 4 $ 的子正方形。压缩操作将 $ s \times s \times c $ 张量转换为 $ \frac{s}{2} \times \frac{s}{2} \times 4 c $ 张量（见 **Fig. 3**），有效地将**空间大小**换成了**通道数**。
...
在每个尺度上，我们将几个操作组合为一个序列：首先使用交替棋盘模板应用三个耦合层，然后执行压缩操作，最后使用交替的 channel-wise 模板应用三个耦合层。
...
选择 channel-wise 模板，以便得到的分区与先前的棋盘模板（见 **Fig. 3**）。对于最终尺度，我们仅应用四个具有交替棋盘模板的耦合层。
...
在计算和存储成本方面以及在需要训练的参数数量方面，在所有耦合层中传播 $D$ 维向量将很麻烦。 

因此，我们遵循[57]的设计选择，并定期将尺寸缩小一半（见[Eq14](#eq14)）。 我们可以递归定义此操作（见**Fig. 4b**）<span id="eq14"></span>
$$
\begin{equation}
 \begin{aligned} 
 h^{(0)} &=x \\
...
**Fig. 4b** 排除变量。在每个步骤中，一半变量直接建模为高斯模型，而另一半则进行进一步的转换。

In our experiments, we use this operation for i < L . The sequence of coupling-squeezing-coupling operations described above is performed per layer when computing f (i) (Equation 14). 
...
在我们的实验中，我们对 $i<L$ 使用此运算。 当计算 $f(i)$（[Eq14](#eq14)）时，每层执行上述**耦合-压缩-耦合**操作的序列。

在每一层，随着空间分辨率的降低，$s$ 和 $t$ 中隐藏层特征的数量将增加一倍。连接所有按不同比例分解的变量，以获得最终的转换输出（[Eq16](#eq14)）。



As a consequence, the model must Gaussianize units which are factored out at a finer scale (in an earlier layer) before those which are factored out at a coarser scale (in a later layer). 

This results in the definition of intermediary levels of representation [ 53 , 49 ] corresponding to more local, fine-grained features as shown in Appendix D.

作为结果，该模型必须对以较细规模（在较早层中）分解出的单位进行高斯化，然后再将其以较粗规模（在较后层中）分解的单位进行高斯化处理。

如 [Appendix D](#appendixd) 所示，这导致定义了表示级别[53、49]的中间级别，该级别对应于更局部的细粒度特征。
...
It also reduces significantly the amount of computation and memory used by the model, allowing us to train larger models.

此外，遵循类似于使用中间分类器引导中间层的原理，在较早的层中进行高斯化和分解单元具有在整个网络中分配损失函数的实际好处。

它还显着减少了模型使用的计算量和内存量，从而使我们能够训练更大的模型。
...
像[22]中一样，我们进一步尝试通过消融测试来掌握我们所学层的潜在变量的语义。 

我们推断潜在向量并从标准高斯重新采样最低水平的潜在向量，从而增加了受此重采样影响的最高水平。

如下图所示，潜在空间的语义似乎更多是在图形级别而不是更高级别的概念。 
