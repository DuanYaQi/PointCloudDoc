...
docker run --runtime=nvidia --rm -it -v /home/duan/data:/home/data -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=unix$DISPLAY -e GDK_SCALE -e GDK_DPI_SCALE -w /home/unknownue/PU-Flow-Condition/PU-Flow unknownue/pu-flow:latest
...
```
docker cp ./ beb9cb3564cf:/home/unknownue/
```
...
```
docker ps -a
docker commit -a "duan" -m "xxxx" -p d07ef4c3bde9 unknownue/pu-flow:latest
```
...
```
1. processing/pugan.py
   def process_pugan_by_matching(h5_source, h5_target):
   
   permutebymatching(lr_batch, sr_batch, k=4, n_grid=3)   k的值要调
   permute.py
   
2. 跑modules/sr/puflow.py
   class ParallelSRFlow(nn.Module):
3. evaluate_pugan.py 
   xyz生成txt文件，需要45min
4. 跑 pugan 里边 evaluate.py 出 csv 文件
...
```bash
[Open3D ERROR] GLFW Error: X11: Failed to open display unix:0
            
宿主机需要运行
xhost +
```

![image-20201104173642322](./assets/image-20201104173642322.png)
...
![image-20201019214842047](./assets/image-20201019214842047.png)



a230667647096224cc06c3fecd6eaeea14788538-PatchSRFlow-resort4-100epoch.ckpt

![image-20201019220337548](./assets/image-20201019220337548.png)



4577251581f7b1fe1dea6f6320002e46ba1348b4-PatchSRFlow-resort4-100epoch.ckpt

![image-20201020125341382](./assets/image-20201020125341382.png)
...
class PointSRFlow(nn.Module):
    ...
    def __init__(self, pc_channel: int, levels: int):				#pc_channel = 3 通道数; levels = 2 上采样层数
...
class GaussianDistribution(Distribution):
    ...
    def __init__(self, pc_channel: int, mu: float, vars: float, temperature: float=1.0):
        ...
        self.prior = MultivariateNormal(mu, vars)                   #先验分布：多元高斯分布 库函数
