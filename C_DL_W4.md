...
对于输出中的每个数字来说，都需要执行 `5×5×192` 次乘法运算，所以乘法运算的总次数为每个输出值所需要执行的乘法运算次数 `5×5×192` 乘以输出值个数 `28×28×32`，把这些数相乘结果等于 120 M。**也可以这样理解**，要将输入的 `28x28x192` 个数字都做一遍卷积，卷积大小为 `5x5`，所以计算一个输出，需要25个自己及周边的值相乘。然后同样的操作需要做 32 遍（滤波器数目）， 即`28x28x192 x 5x5 x 32`。
